...
def train_model():
    ...
    args = {'architecture': 'mobilenet_1.0_224',
            ...
            'output_labels': tf_files_path + '/tf_files/retrained_labels.txt',
            'print_misclassified_test_images': False, 'random_brightness': 0, 'random_crop': 0, 'random_scale': 0,
            'summaries_dir': tf_files_path + '/tf_files/training_summaries/mobilenet_1.0_224',
            'test_batch_size': -1, 'testing_percentage': 10, 'train_batch_size': 100, 'validation_batch_size': 100,
            'validation_percentage': 10}

    logging.info('User parameters:')
    logging.info(args)
    flags = Namespace(**args)
    scripts.retrain.FLAGS = flags
    scripts.retrain.main(args)
    logging.info('Model training finnished')
...
def load_model():
    try:
        ...
        global_labels = load_labels(tf_files_path + '/tf_files/retrained_labels.txt')
    except FileNotFoundError:
        logging.error('Model or labels file not found')
